...
% %改进的销量矩阵
% [groups, ~, idx] = unique(data_23(:, {'plant_id', 'planted_time'}), 'rows');
% constr_sales = accumarray(idx, data_23.data_yield);
% result = [groups table(constr_sales, 'VariableNames', {'SummedValue'})];
...
constr_cost_acre_n=zeros(41,82);
constr_price=constr_price.data_price;
...
for year_now = 1:7
    ...
    %销售单价
    constr_price_n(1:5,1)=constr_price(1:5).*(1-0.3254*(rand(1,1)*0.1-0.05));
    constr_price_n(8:16,1)=constr_price(8:16).*(1-0.3254*(rand(1,1)*0.1-0.05));
    constr_price_n(6:7,1)=constr_price(6:7).*(1-0.3254*(rand(1,1)*0.05+0.05));
    constr_price_n(17:37,1)=constr_price(17:37).*(1-0.5158*(rand(1,1)*0.1-0.05));
    constr_price_n(38:41,1)=constr_price(38:41).*(1-0.6917*(rand(1,1)*0.1-0.05));
    %种植成本
    constr_cost_acre_n(1:5,:)=constr_cost_acre(1:5,:).*(1+0.9325*(rand(1,1)*0.1-0.05));
    constr_cost_acre_n(8:16,:)=constr_cost_acre(8:16,:).*(1+0.9325*(rand(1,1)*0.1-0.05));
    constr_cost_acre_n(6:7,:)=constr_cost_acre(6:7,:).*(1+0.9325*(rand(1,1)*0.05+0.05));
    constr_cost_acre_n(17:37,:)=constr_cost_acre(17:37,:).*(1+0.7835*(rand(1,1)*0.1-0.05));
    constr_cost_acre_n(38:41,:)=constr_cost_acre(38:41,:).*(1-0.2774*(rand(1,1)*0.1-0.05));
    ...
    % 水浇地水稻约束
    % for id_land = 27:34
    %     constr = [constr, implies(year_true(16,id_land)==1, sum(year_true(17:34,id_land)) == 0)];
    %     constr = [constr, implies(year_true(16,id_land)==0, sum(year_true(17:34,id_land)) >= 1)];
    %     constr = [constr, sum(year_true(:,id_land)) >= 0, sum(year_true(:,id_land)) <= 2];
    % end
    ...
    %%
    %目标函数
    % obj = - (sum( min( sum( (year_plan .* constr_prop .* constr_yield_acre .* constr_area) ,2 ) , constr_sales ) .* constr_price) ...
    ...
    constr_cost_acre=constr_cost_acre_n;
    constr_price=constr_price_n;
end
disp('end');
