...
for year_now = 1:7
    ...
    obj = - (sum( min( year_rev, constr_sales ) .* constr_price.data_price ) + ...
             sum( abs( year_rev - min( year_rev, constr_sales )) * 0.5 .* constr_price.data_price ) ...
             - sum(sum(year_plan .* constr_prop .* constr_area .*constr_cost_acre )));
    ...
    % objective = sum( min( sum( (result .* constr_yield_acre .* constr_area) ,2 ) , constr_sales ) .* constr_price.data_price) ...
    %      - sum(sum(result .* constr_area .*constr_cost_acre));

    year_rev_renew = sum( (result .* constr_yield_acre .* constr_area) ,2 );
    objective = (sum( min( year_rev_renew, constr_sales ) .* constr_price.data_price ) + ...
             sum( abs( year_rev_renew - min( year_rev_renew, constr_sales )) * 0.5 .* constr_price.data_price ) ...
             - sum(sum( result .* constr_area .*constr_cost_acre )));
